...
Order lifecycle rules keep changing. Marketing wants to add a "pending review" step for high-value orders. Operations wants to prevent cancellation after shipping. Compliance needs an audit trail of every accepted state transition.
...
- Re-entrant `send()` from within an action.
- `available_events()` or listing valid events from the current state.
- Export of `Transition` from the public API.
- History size limits or pruning.
...
A transition maps an event from one or more source states to a destination state. It may optionally carry a guard condition and an action. Self-transitions (`to_state` equal to a source state) are allowed.
...
An action is a Python callable invoked with the context dict when a transition fires, before the state is updated. If the action raises any exception, the state is not updated and the transition is rejected. *(per `requirements.pdf` §4, §7.2, §8)*

If the action raises, only `current_state` and `history` are protected: any mutations the action performed before the exception (context changes, external I/O) are **not** rolled back.
...
If any segment is missing or non-traversable, the evaluator raises `EvaluationError`. *(per `requirements.pdf` §5, §7.2, §8)*
...
- Raises `ValueError` if `from_state` normalises to an empty list.
- Guard is parsed and validated immediately using the condition grammar. Raises `ValueError` if the guard is syntactically invalid. The error message includes the transition name (see [Rejection reason templates](#rejection-reason-templates)).
- Raises `ValueError` if a transition with the same name already covers any of the same source states (overlap check). *(per `requirements.pdf` §7.1)*
...
1. If `context` is not a `dict`: return `TransitionResult(accepted=False, reason="Invalid context: context must be a dict")`.
2. Find all registered transitions where `t.name == event` and `sm.current_state in t.from_states`.
3. If none found: return `TransitionResult(accepted=False, reason="No transition '{event}' from state '{current_state}'")`.
4. Take the first match (registration order).
5. If the transition has a guard:
   - Evaluate the parsed guard against `context`.
   - If `EvaluationError` (e.g. missing field): return `TransitionResult(accepted=False, reason="Guard error: {message}")`. *(per `requirements.pdf` §7.2, §8)*
   - If guard is `False`: return `TransitionResult(accepted=False, reason="Guard condition not met: {guard}")`.
6. If the transition has an action:
   - Call `action(context)`.
   - If the action raises any exception: return `TransitionResult(accepted=False, reason="Action error: {message}")`. State does not change. *(per `requirements.pdf` §7.2, §8)*
7. Update `sm.current_state` to `to_state`.
8. Append a `HistoryEntry` to internal history.
9. Return `TransitionResult(accepted=True, ...)`.

`send()` never raises. All operational failures are captured in the result, including invalid `context`.

### `can()`

```python
sm.can(event, context=None)
```

Returns `True` if a matching transition exists for the current state and either no guard is defined or the guard evaluates to `True`. Does **not** invoke actions — so `can()` may return `True` when `send()` would reject due to action failure. Does not modify state or history. If `context` is `None`, treats it as `{}`. If `context` is not a `dict`, returns `False`. If guard evaluation raises `EvaluationError`, returns `False`.
...
Sets `current_state` to `initial` (or the given state) and clears history. When a state argument is given, it must be in `sm.states`; otherwise raises `ValueError`. Does not remove registered transitions.

### `history` property

Returns a **copy** of the internal history list. Only accepted transitions are recorded; rejected transitions are not included. Mutating the returned list does not affect the machine.
...
All `TransitionResult.reason` strings for rejected `send()` calls follow these templates exactly (`{…}` denotes interpolated values):

| Rejection path | `reason` template |
| --- | --- |
| Invalid context | `Invalid context: context must be a dict` |
| No matching transition | `No transition '{event}' from state '{current_state}'` |
| Guard `EvaluationError` | `Guard error: {message}` — `{message}` is `str(exception)` |
| Guard evaluates to `False` | `Guard condition not met: {guard}` — `{guard}` is the guard string |
| Action raises | `Action error: {message}` — `{message}` is `str(exception)` |

Registration-time guard syntax errors raise `ValueError` with message:

`Invalid guard for transition '{name}': {detail}`

where `{detail}` is the underlying parse error text.

### Error handling

| Scenario | When | Behaviour |
| --- | --- | --- |
| Invalid guard syntax | `sm.transition()` | Raises `ValueError` immediately; message includes transition name |
| Empty `from_state` | `sm.transition()` | Raises `ValueError` immediately |
| Duplicate `(name, from_state)` overlap | `sm.transition()` | Raises `ValueError` immediately *(per `requirements.pdf` §7.1)* |
| Unknown state | `sm.reset(state)` | Raises `ValueError` immediately |
| Invalid `context` (not a `dict`) | `sm.send()` | Returns `TransitionResult(accepted=False)` |
| No matching transition | `sm.send()` | Returns `TransitionResult(accepted=False)` |
| Guard field missing from context | `sm.send()` evaluation | Returns `TransitionResult(accepted=False, reason="Guard error: …")` *(per `requirements.pdf` §8)* |
| Guard evaluates to `False` | `sm.send()` evaluation | Returns `TransitionResult(accepted=False, reason="Guard condition not met: …")` |
| Action raises any exception | `sm.send()` action call | Returns `TransitionResult(accepted=False, reason="Action error: …")`. State not updated *(per `requirements.pdf` §8)* |
...
`Transition` is an internal data class and is not part of the public API.
...
One entry per accepted transition, in order. Rejected transitions are not recorded.
...
- `send()` must never raise; all operational failures are returned via `TransitionResult`, including invalid `context`.
...
- `can()` checks transition match and guard only; it does not invoke actions.
- `can()` must not modify state or history.
- `reset()` clears history but preserves registered transitions; `reset(state)` must validate against `sm.states`.
- `history` returns a copy; only accepted transitions are recorded.
- Self-transitions are allowed.
- Empty `from_state` is rejected at registration time.
- v1 excludes named state declarations, entry/exit actions, persistence, parallel states, nested states, async execution, thread safety, re-entrant `send()`, `available_events()`, `Transition` export, history pruning, and diagram export (see [Out of scope](#out-of-scope-v1)).
...
assert result.reason == "Action error: fail"
...
result = sm.send("submit", None)
assert result.accepted is False
assert result.reason == "Invalid context: context must be a dict"
assert sm.current_state == "draft"
```
...
- `sm2.history` returns a copy: mutating the returned list does not change subsequent reads of `sm2.history`.

### Registration and API constraints

- Invalid guard syntax at `sm.transition()` raises `ValueError` matching `Invalid guard for transition '{name}': …`.
- Overlapping `(name, from_state)` registration raises `ValueError`.
- Empty `from_state` (e.g. `[]`) raises `ValueError`.
- `from_state` accepts either a single string or a list of strings (e.g. cancel from `["draft", "submitted", "processing"]`).
- Self-transitions (e.g. `from_state="draft"`, `to_state="draft"`) are accepted.
- `sm.reset("unknown")` raises `ValueError` when `"unknown"` is not in `sm.states`.
- `send()` returns structured rejection results (no uncaught exceptions from guard, action, or invalid context).
